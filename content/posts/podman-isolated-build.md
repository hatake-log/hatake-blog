...
Codablecashのネットワーク公開が近づいてきました。マイニング初心者としてまずやっておきたいのが、「ソースコードから自分でビルドして動かせる」ようになっておくことです。今回はその予行演習として、隔離環境でのビルドから、同梱テストを全部走らせるところまでやってみました。

## 「隔離環境」でビルドする

ビルドするのは、GitHubから取ってきた開発中のソフトウェアです。作者を疑っているわけではなく、実験段階のコードには未知の不具合がありえますし、ビルド手順の中で何が実行されるかを全部読み切る力も私にはまだありません。

そして重要なのは、手元のマシンは**将来ノードを動かし、ウォレットの鍵を扱う予定の機械**だということです。以前の[GDS対策の記事](/posts/gds-mitigation/)と同じ発想で、万一の時の影響範囲を広げないための防御の一枚として、ビルドも実行も「隔離環境」の中で行うことにしました。

## Podmanルートレスという檻

今回は、その隔離環境（檻）として**Podman**を使います。Dockerと同じコンテナの道具ですが、**root権限の常駐サービスなしで、一般ユーザーのまま動かせる**（ルートレス）のが売りです。

ビルドのたびに新しい檻が建ち、終わると丸ごと消えます。それなのにビルドの成果物が消えないのは、ソースコードのフォルダだけを、檻の中に「窓」として見せているから。**檻の中から見えるのはそのフォルダだけ**で、ホームディレクトリなどの外の環境は、檻の中からは見えません。
...
檻の金型（イメージ）は、このContainerfileを同フォルダに保存して作ります。ポイントは、ビルドに必要な道具を全部ここで入れてしまい、ビルド時にはネットワークを不要にすることです。ビルド（cmakeやmake）は実は任意のコマンドを実行できる工程なので、通信を遮断しておけば「ビルド中に外から何かを取ってくる・外へ何かを送る」類の挙動を構造的に封じられます。そして遮断したままビルドが通れば、隠れた外部依存がないことの確認にもなります。
...
この2本立てで実行します。先にお伝えしておくと、ちょっとした問題が発生し、作業はスムーズには進みませんでした。結果的にはちゃんとビルドは成功しましたので、この記事ではその辺りも含めて記録として記載します。
...
２つ目のコマンドでエラーが発生しました。進捗3%ほどで止まっています。

```console
[  3%] Building CXX object CMakeFiles/codablecashlib.dir/src_blockchain/...
/work/src_blockchain/***.cpp: fatal error: ***.h: No such file or directory
compilation terminated.
make[2]: *** [CMakeFiles/codablecashlib.dir/build.make:580: ...] Error 1
make: *** [Makefile:101: all] Error 2
```

（`***`のファイル名は、開発者さんに報告済みです。修正が取り込まれるまで伏せておきます）

こちら調査したところ、**執筆時点のmainブランチは、ビルドが途中で止まる箇所が1つあることがわかりました**。数行の修正で解決しましたので、修正して再実行します。（**修正方法は、開発者さんにお伝えできたタイミングで別の記事にする予定です**）
...
またしても`maketest.sh`の実行でエラーです。ですが、先ほど出たエラーとは違うようです。

```console
---- Block Head -----
 38f6e63a2e6f3a2a92f2bd630c9d1a03f09b936803bb3cd7cb27e818012490c1 [height: 1 voted: 0 voting: 0 mev: -1]
---------------------- at file: /work/src_blockchain/bc_status_cache/BlockHead.cpp, line : 263
****************** SELECTED ******************* at file: /work/src_blockchain/bc_status_cache/HeadBlockDetector.cpp, line : 282
./sh/maketest.sh: line 10:  9688 Segmentation fault      (core dumped) ./testall -v
```

今度はビルドは最後まで通っていて、エラーが出たのはその後のテスト（`./testall`）の途中です。調べてみると、どうやら上流のバグではなく、私の環境由来のようです。

テストには**P2P（ノード同士の通信）の試験が含まれる**ので、通信が遮断された檻では通信が失敗して落ちてしまう。これが原因でした。
...
一旦exitして再度入り直します。コマンドは、以下のように`--network=none`不使用です。
...
コマンド実行後はテストコードがごにょごにょと動き、今度こそ無事に完了！・・・かと思いきや、最後の最後になにやらエラーが出ています。
...
## 仕上げ：テスト2070個の結果を見届ける

カバレッジ集計（lcov）は、テストがコードのどの行を通ったかを集計してくれる仕組みです。新しめのバージョン（Ubuntu 24.04では2.x系）だと検査が厳しく、今回のように途中で止まることがあります。設定ファイルを先に置いてから、もう一度実行します。
...
Testing Summary
  ...
  Checks(success : 10049, failed :0)
```

**テスト2070個、チェック10049回、全部成功**（所要約11分）。さらに集計が終わると、`target/html_report/`にカバレッジレポート（テストがコードのどの行を通ったかの地図）がHTMLで生成されます。檻を出た後、ホスト側からブラウザで開けます。先頭の集計はこうでした。

```text
Lines: 97.1 %  (79413行中 77101行)
```
